...
class ContainerPool:
    ...
    def from_config(cls) -> "ContainerPool":
        ...
        try:
            ...
            return cls(
                image=kali_cfg.get("image", "autovapt-kali:latest"),
    ...
    async def cleanup_orphans(self):
        """Find and remove autovapt-* containers not tracked by this pool."""
        if not HAS_DOCKER:
            return
        ...
        try:
            ...
            containers = client.containers.list(
                all=True,
                filters={"label": "autovapt.type=kali-sandbox"},
            )
            async with self._lock:
            ...
            removed = 0
            for c in containers:
                scan_id = c.labels.get("autovapt.scan_id", "")
