...
![AutoVAPT](https://img.shields.io/badge/AutoVAPT-AI--Powered%20Pentesting-blueviolet)
...
```bash
# Clone repository
git clone https://github.com/CyberSecurityUP/AutoVAPT/
cd AutoVAPT
...
ContainerPool (global coordinator, max 5 concurrent)
  ├── KaliSandbox(scan_id="abc") → docker: autovapt-abc
  ├── KaliSandbox(scan_id="def") → docker: autovapt-def
  └── KaliSandbox(scan_id="ghi") → docker: autovapt-ghi
```
...
AutoVAPT uses a multi-layered validation pipeline to eliminate false positives:
...
# Database
DATABASE_URL=sqlite+aiosqlite:///./data/autovapt.db
...
**AutoVAPT v1** - *AI-Powered Autonomous Penetration Testing Platform*
