"""
AutoVAPT v1 - Logic and Protocol Vulnerability Testers
...
class ParameterPollutionTester(BaseTester):
    ...
    def analyze_response(
        ...
        if "autovapt" in payload and "autovapt" in response_body:
...
class HostHeaderInjectionTester(BaseTester):
    ...
    def analyze_response(
        ...
        evil_hosts = ["evil.com", "attacker.com", "autovapt.test", "canary.host"]
...
class CachePoisoningTester(BaseTester):
    ...
    def analyze_response(
        ...
        if cache_hit:
            ...
            injection_markers = ["autovapt", "xss", "evil.com", "attacker"]
        ...
        for header in unkeyed_headers:
            if header in payload.lower():
                ...
                for marker in ["evil.com", "autovapt", "attacker"]:
                    if marker in payload.lower() and marker in response_body.lower():
