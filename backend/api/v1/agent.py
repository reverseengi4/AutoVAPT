"""
AutoVAPT v1 - AI Agent API Endpoints
...
async def get_llm_status():
    ...
    if openai_key in ["", "your-openai-api-key"]:
        openai_key = None
    if gemini_key in ["", "your-gemini-api-key"]:
        gemini_key = None
    ...
    # Check if requests library is available (needed for Gemini HTTP client)
    try:
        import requests
        requests_lib = True
    except ImportError:
    ...
    elif gemini_key and requests_lib:
        status = "ready"
        provider = "gemini"
        message = "Gemini API configured and ready"
    elif not all([anthropic_lib, openai_lib, requests_lib]):
        status = "error"
        provider = None
        message = "Missing dependencies. Run: pip install anthropic openai requests"
    else:
        status = "not_configured"
        provider = None
        message = "No API key configured. Set ANTHROPIC_API_KEY, OPENAI_API_KEY, or GEMINI_API_KEY"
    ...
    return {
        ...
        "details": {
            ...
            "requests_lib_installed": requests_lib
        }
    }
...
async def create_realtime_session(request: RealtimeSessionRequest):
    ...
    realtime_sessions[session_id] = {
        ...
        "messages": [
            {
                "role": "system",
                "content": f"""You are AutoVAPT, an AI-powered security testing assistant.
