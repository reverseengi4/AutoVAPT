  ...
  const handleGenerateReport = async (format: 'json' | 'html' = 'json') => {
    ...
    try {
      if (format === 'html') {
        ...
        a.href = url
        a.download = `autovapt-report-${agentId}-${new Date().toISOString().split('T')[0]}.html`
        a.click()
        ...
        const a = document.createElement('a')
        a.href = url
        a.download = `autovapt-report-${agentId}-${new Date().toISOString().split('T')[0]}.json`
        a.click()
        URL.revokeObjectURL(url)
      }
    } finally {
  ...
  const generateHTMLReport = () => {
    ...
    <div style="background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); border: 1px solid #334155; border-radius: 12px; padding: 24px; margin-bottom: 40px;">
      ...
      <p style="color: #cbd5e1; line-height: 1.8; margin: 0 0 20px;">
        This security assessment of <strong style="color: white;">${status.target}</strong> was conducted using AutoVAPT AI-powered penetration testing platform.
...
<head>
  ...
  <title>AutoVAPT Security Report - ${agentId}</title>
  <style>
...
<body>
  <div class="container">
    <div class="header">
      <h1>🛡️ AutoVAPT Security Assessment Report</h1>
    ...
    <div class="footer">
      <p><strong>Generated by AutoVAPT v1.0 AI Security Scanner</strong></p>
    ...
    <div className="p-4 pt-0 space-y-4 border-t border-dark-700">
      ...
      <div className="flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <span className={`font-bold ${finding.cvss_score >= 9 ? 'text-red-500' :
        ...
        {finding.confidence && (
          <span className={`text-xs px-2 py-1 rounded ${finding.confidence === 'high' ? 'bg-green-500/20 text-green-400' :
      ...
      {logsToShow.length === 0 ? (
        ...
        logsToShow.map((log, i) => {
          ...
          return (
            <div
              key={i}
              className={`flex gap-2 py-1 px-1 rounded ${isUserPrompt ? 'bg-blue-500/10 border-l-2 border-blue-500' :
            ...
            >
              ...
              <span className={`break-words ${isUserPrompt ? 'text-blue-300 font-medium' :
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="flex items-center gap-3 mt-2">
            <span className={`px-3 py-1 rounded-full text-sm font-medium flex items-center gap-1 ${status.status === 'running' ? 'bg-blue-500/20 text-blue-400' :
        ...
        <Card>
          <div className="space-y-4">
              ...
              {SCAN_PHASES.map((phase, index) => {
                ...
                return (
                  <div key={phase.key} className="flex flex-col items-center flex-1 relative group">
                    {/* Connector line */}
                    {index > 0 && (
                      <div className={`absolute top-4 right-1/2 w-full h-0.5 -translate-y-1/2 z-0 ${isCompleted || isActive ? 'bg-green-500/50' :
                    ...
                    {/* Phase node */}
                    <div
                      className={`relative z-10 w-8 h-8 rounded-full flex items-center justify-center mb-1 transition-all ${isSkipped ? 'bg-yellow-500/20 text-yellow-500 ring-2 ring-yellow-500/30' :
                    ...
                    <span className={`text-xs text-center ${isSkipped ? 'text-yellow-500' :
            ...
            <div className="h-2 bg-dark-900 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-500 ${status.status === 'completed' ? 'bg-green-500' :
      ...
      {status.status === 'running' && (
        <Card>
          <div className="space-y-3">
            ...
            {promptSentMessage && (
              <div className={`flex items-center gap-2 text-sm ${promptSentMessage.includes('Failed') ? 'text-red-400' : 'text-green-400'
                }`}>
